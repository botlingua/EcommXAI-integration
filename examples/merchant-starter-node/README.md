# `ecommxai-merchant-starter-node`

> EcommX AI 商家整合 starter — Node.js + Express。
>
...
完整 OAuth flow（authorize → token → userinfo）見 `src/oauth/routes.js`。

---

## 接到你的會員系統

1. **`src/middleware/auth.js`** — 換成你的 Bearer 驗證邏輯（從 KMS / Vault 拿 token 比對）
2. **`src/commerce/data.js`** — 改成你自家 DB query（products / categories / inventory 對齊 §3 shape）
3. **`src/oauth/store.js`** — `authenticateMember()` 換成自家 user table hash compare；`MEMBERS` 換 DB 查詢
4. **`src/oauth/jwt.js`** — `generateKeyPairSync` 換成從 KMS / file 載你自家 RSA private key（**勿** 每次重啟 rotate；buyer 重綁所有舊 access_token 會失效）
5. **`src/oauth/routes.js`** — `DEMO_CLIENT` 換成自家 OAuth client 註冊表（platform 端 client_id / client_secret 對齊）
...
- [ ] OAuth RSA private key 從 PEM file 載入（用 `crypto.createPrivateKey(fs.readFileSync(...))`）；勿每次重啟 rotate
- [ ] `OAUTH_ISSUER` 改成 prod domain (`https://api.acme.com`)
- [ ] 在 prod 反向代理（nginx / Caddy / Cloudflare）上掛 TLS cert
- [ ] Authorization code store 改 Redis（in-memory 跨 pod 不共享）
